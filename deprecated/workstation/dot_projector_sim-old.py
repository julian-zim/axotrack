...
def main():
	...
	image_bg_path = os.path.join('.', 'data', 'sim', 'background.jpg')
	...
	projector_width, projector_height = image_bg_width, image_bg_height		# the background image should have the same dimensions as the second screen
	...
	txt_do_path = os.path.join(os.getcwd(), 'data', 'sim', 'dot_offset.txt')
	...
	while i_c < num_imgs:
		...
		image_diff_track = cv2.polylines(image_diff_track, [twz_pts], True, (127, 127, 127), 2)
		image_diff_track_line = cv2.line(image_diff_track, [round(c) for c in center], [round(c) for c in dot], color_mono, 1)
		image_diff_track_dot = cv2.circle(image_diff_track_line, [round(c) for c in dot], round(size), color_mono, -1)

		# show
		cv2.imshow(trackwindow_name, image_track_dot)
		cv2.setMouseCallback(trackwindow_name, mouse_callback)
		# placeholder
		cv2.imshow(diff_window_name, image_diff_track_dot)
